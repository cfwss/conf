...
- Supported ChatGPT 检测

[task_local]

event-interaction https://raw.githubusercontent.com/cfwss/conf/main/QuantumultX/streaming-ui-check.js, tag=Streaming-Unlock Query, img-url=checkmark.seal.system, enabled=true
...
let result = {
  "title": '   Streaming Unlock Query',
  "YouTube": '<b>YouTube: </b>Failed, Try again ❗️',
  "Netflix": '<b>Netflix: </b>Failed, Try again ❗️',
  "Dazn": "<b>Dazn: </b>Failed, Try again ❗️",
  "Disney": "<b>Disneyᐩ: </b>Failed, Try again ❗️",
  "Paramount" : "<b>Paramountᐩ: </b>Failed, Try again ❗️",
  "Discovery" : "<b>Discoveryᐩ: </b>Failed, Try again ❗️",
  "ChatGPT" : "<b>ChatGPT: </b>Failed, Try again ❗️"
  //"Google": "Google 定位: Failed, Try again"
...
;(async () => {
  ...
  if (status==STATUS_COMING) {
    ...
    result["Disney"] = "<b>Disneyᐩ:</b> Detect timeout 🚦 "
  }
  ...
  content = content + "</br>------------------------------</br>"+"<font color=#CD5C5C >"+"<b>Endpoint</b> ➟ " + $environment.params+ "</font>"
...
$configuration.sendMessage(message).then(resolve => {
    ...
    if (resolve.ret) {
      ...
      content = content + "</br>--------------------------------------</br>"+"<font color=#CD5C5C>"+"<b>Endpoint</b> ➟ " + output+ "</font>"
      content =`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + content + `</p>`
...
.finally(() => {
  ...
  $configuration.sendMessage(message).then(resolve => {
    ...
    $done({"title":result["title"],"htmlMessage":`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">`+'----------------------</br></br>'+"🚥 Detect anomalies"+'</br></br>----------------------</br>'+ output + `</p>`})
}
  );
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response => {
      ...
      if (response.statusCode === 404) {
        ...
        result["Netflix"] = "<b>Netflix: </b>Supported homemade ⚠️"
        ...
        result["Netflix"] = "<b>Netflix: </b>Full supported"+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
        //$notify("nf:"+result["Netflix"])
      ...
      result["Netflix"] = "<b>Netflix: </b>Detect timeout 🚦"
...
function testYTB() { 
    ...
    $task.fetch(option).then(response=> {
      ...
      if (response.statusCode !== 200) {
        //reject('Error')
        result["YouTube"] = "<b>YouTube Premium: </b>Detection failed ❗️"
      ...
      //resolve(region)
      result["YouTube"] = "<b>YouTube Premium: </b>Supported "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      console.log("ytb:"+region+ result["YouTube"])
      }
    }, reason => {
      result["YouTube"] = "<b>YouTube Premium: </b>Detect timeout 🚦"
      //resolve("timeout")
    })
}
...
function testDazn() { 
  ...
  $task.fetch(option).then(response=> {
    ...
    if (response.statusCode !== 200) {
      //reject('Error')
      result["Dazn"] = "<b>Dazn: </b>Detection failed ❗️"
      ...
      if (ret != null && ret.length === 2) {
        region = ret[1]
        result["Dazn"] = "<b>Dazn: </b>Supported "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
      } else {
    ...
    result["Dazn"] = "<b>Dazn: </b>Detect timeout 🚦"
    //resolve("timeout")
  })
}
...
function testParam() { 
  ...
  $task.fetch(option).then(response=> {
    ...
    if (response.statusCode == 200) {
      //reject('Error')
      result["Paramount"] = "<b>Paramountᐩ: </b>Supported 🎉 "
    ...
    result["Paramount"] = "<b>Paramountᐩ: </b>Detect timeout 🚦"
    //resolve("timeout")
  })
}
  ...
  return new Promise((resolve, reject) =>{
    ...
    $task.fetch(option).then(response=> {
      ...
      $task.fetch(option1).then(response=> {
        ...
        if (res != -1) {
          result["ChatGPT"] = "<b>ChatGPT: </b>Supported "+arrow+ "⟦"+flags.get(region.toUpperCase())+"⟧ 🎉"
          console.log("Supported ChatGPT")
          resolve("Supported ChatGPT")
          return
        } else {
